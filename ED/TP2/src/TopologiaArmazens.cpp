...
PilhaPacotes* SecoesArmazem::InsereSecao(int idSecao) {
    ...
    TipoCelula* novaCelula = new TipoCelula(novaPilha);
...
TopologiaArmazens::TopologiaArmazens(int numeroVertices, int** matrizAdjacencia) {
    this->tamanho = 0;
...
void TopologiaArmazens::InsereAresta(int idV, int idW){
    ...
    secoesV->InsereSecao(idW);
    secoesW->InsereSecao(idV);
}
