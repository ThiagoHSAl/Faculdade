...
function startSearch() {
    ...
    // 1. Pega o elemento <select> do mapa
    const mapSelect = document.getElementById('maps');
    // 2. Pega o TEXTO da opção selecionada (que é o nome do mapa)
    const mapName = mapSelect.options[mapSelect.selectedIndex].text;

    // 3. Constrói a URL com o novo parâmetro 'map_name'
    var url = `http://localhost:5001/start_search?map=${encodeURIComponent(map)}&alg=${encodeURIComponent(alg)}&heuristic=${encodeURIComponent(heuristic)}&map_name=${encodeURIComponent(mapName)}`;
